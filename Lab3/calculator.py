...
def equals(in_str):
    try:
        out_str = equal(in_str)
        entry.delete(0, tk.END)
        entry.insert(0, out_str)
    except:
...
button_equals = tk.Button(master=frame, text="=", padx=15, pady=5, width=9, command=lambda: equals(entry.get()))
button_equals.grid(row=7, column=0, columnspan=3, pady=2)

button_exit = tk.Button(master=frame, text="Close", padx=15, pady=5, width=9, command=window.destroy)
button_exit.grid(row=8, column=0, columnspan=3, pady=2)
